<?php
/**
 * Administator's page for managing objective types.
 *
 *	 Requirements:
...
function bs_objectivetypes( $bsdriver, $bsuser )
{
	$configPage = new BsObjectiveTypesPage(
		$bsdriver,
		$bsuser,
		__( 'Objective Types' ),	// Parm: Title
		...
		__( 'Are you sure you want to delete this objective type?' )
		);
	return $configPage->Response();
}

/**
 * Objective types processing within a class.
 *
 * @package objtracker
 * @category Class
 * @author  Dan Roller <bsdanroller@gmail.com>
 */
class BsObjectiveTypesPage extends ObjtrackerConfigPage
{
	/**
	...
	)
	{
		...
		$this->db_messages = array(
			'TypeUpdTitle'      => __( 'Title is required' ),
			'TypeUpdDesc'       => __( 'Description is required' ),
			'TypeUpdDupTitle'   => __( 'Title already exists' ),
			'TypeUpdDupDesc'    => __( 'Description already exists' ),
			);
	}
	...
	function description()
	{
		...
		$this->description_text( __( 'The table below lists the objective types that are assigned to objectives.' ) );
		...
		if ( $bsuser->is_admin() ) {
			$this->description_list_item(
			__(
				'To edit a objective type, click on <b>Edit</b>, change values, and click on <b>Update</b>.'
				)
			);
			$this->description_list_item(
			__(
		...
		$this->_Description .= $bsdriver->title_heading( __( 'Objective Types' ) );
	}
	...
	function row_update()
	{
		...
		} elseif ( !$this->is_valid_dbparm( 128, __( 'Descrition' ), $C_Description ) ) {
		} else {
			$this->db_change(
				'P_ObjectiveTypeUpdate',
				'( %d, %d, %s, %s, %s )',
	...
	function gridview_row_list( $row )
	{
		...
		$usageLink = ( $C_Usage == 0 )
			...
			: "<a href='" . $bsdriver->PathBase . $bsdriver->PlatformParm
				. 'sc_menu=Usage&Table=T_Objective&Column=TypeID&Value=' . $C_ID . "'>" . __( 'Yes:' ) . $C_Usage . '</a>';
